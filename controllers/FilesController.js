...
class FilesController {
  ...
  static async postUpload(req, res) {
    ...
    const resultData = await dbClient.insertFile(file);
    const fQueue = new Bull('fQueue');
    await fQueue.add({ fileId: resultData.id, userId: user.id });
    return res.status(201).send(resultData);
  }
}
